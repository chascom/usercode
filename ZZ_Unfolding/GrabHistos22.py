...
def makeComparison(MCGenHistos,MCRecoHistos,hTrues,hDiffs,hCloses,pref):
	...
	hClose.GetYaxis().SetTitle("#frac{d#sigma}{dE}(pp #rightarrow ZZ #rightarrow 2l2#nu) [fb/GeV]")
	hClose.GetYaxis().SetTitleOffset(1.2)
	hClose.GetYaxis().SetLabelSize(.04)
	hClose.GetXaxis().SetLabelSize(.04)
	...
	leg = TLegend(0.55,0.75,0.62,0.88,"","brNDC")
	leg.SetTextFont(132)
	...
	upstat = UD2[2]
	dnstat = UD2[3]
	binc = BinContent(hTrue)

	binc = numpy.add(binc,[0,0,0,0,0.00001])

	fliplast = [1,1,1,1,1]
	upsys = numpy.multiply(numpy.divide(UD2[0],binc),fliplast)
	dnsys = numpy.multiply(numpy.divide(UD2[1],binc),fliplast)
	upstat = numpy.multiply(numpy.divide(UD2[2],binc),fliplast)
	dnstat = numpy.multiply(numpy.divide(UD2[3],binc),fliplast)

	# upsys = numpy.divide(UD2[0],binc)
	# dnsys = numpy.divide(UD2[1],binc)
	# upstat = numpy.divide(UD2[2],binc)
	# dnstat = numpy.divide(UD2[3],binc)
	#getbincontent -> divide UD/true
...
#[109.01558259502053, 69.720078293586326, 38.537151666511029]
	...
	h_comp4.GetYaxis().SetLabelSize(.09)
	h_comp4.GetXaxis().SetLabelSize(.09)
	h_comp4.GetYaxis().SetTitle("Frac. Uncert.")
	h_comp4.Draw("SAME")
	#################################################################
	#################################################################
	# leg4 = TLegend(0.65,0.1,0.89,0.88,"","brNDC")
	...
	# leg4.Draw("SAME")
	leg4 = TLegend(0.65,-0.4,0.89,-0.1,"","brNDC")
	leg4.SetTextFont(132)
	leg4.SetTextSize(0.15)
	leg4.SetFillColor(0)
	leg4.SetBorderSize(0)
