...
def Ratios(UP,DOWN,HistoN,HistoM): #N/M propagation of asymmetrical error
	MM = BinContent(HistoM)
	NN = BinContent(HistoN)
	ERR = BinError(HistoM)
...
def makeComparison(MCGenHistos,MCRecoHistos,hTrues,hDiffs,hCloses,pref):
	...
	leg = TLegend(0.65,0.75,0.80,0.88,"","brNDC")
	leg.SetTextFont(132)
	leg.SetTextSize(0.035)
	leg.SetFillColor(0)
	leg.SetBorderSize(0)
	leg.AddEntry(MCGenHisto,"gen MC (ZZ)")
	leg.AddEntry(MCRecoHisto,"RecoMC*(UData/GMC="+str(clos[-1])[:4]+")")
	...
	h_comp3.GetYaxis().SetTitleSize(.18)
	...
	h_comp4.GetYaxis().SetTitleSize(.18)
