...
Float_t BDTZH135;
reader->AddVariable("BDTZH135",&BDTZH135);

Float_t LikelihoodZH135;
reader->AddVariable("LikelihoodZH135",&LikelihoodZH135);

Float_t BDTZZ;
reader->AddVariable("BDTZZ",&BDTZZ);

Float_t LikelihoodZZ;
reader->AddVariable("LikelihoodZZ",&LikelihoodZZ);
...
//      reader->AddSpectator( "Category_cat3 := (var3>0)&&(var4>=0)", &Category_cat3 );
 ...
 float nonsense =0;
   ...
   //   
   TFile *input(0);
   TString fname = "/tmp/chasco/INIT/HADD/TMVA/Trees_FUSION2/ZZ_vs_nonZZ/ZH105_vs_ZZ/ZH115_vs_ZZ/ZH125_vs_ZZ/ZH150_vs_ZZ/ZH135_vs_ZZ/ZH145_vs_ZZ//Data7TeV_DoubleMu2011B_1.root";   
   ...
   //
   TTree* BigTree = (TTree*)input->Get("tmvatree");

TFile *tmp  = new TFile( "tmp.root","RECREATE" );

TTree* theTree = BigTree->CopyTree("(CUT>2)");
...
theTree->SetBranchAddress( " BDTZH135", &BDTZH135);

theTree->SetBranchAddress( " LikelihoodZH135", &LikelihoodZH135);

theTree->SetBranchAddress( " BDTZZ", &BDTZZ);

theTree->SetBranchAddress( " LikelihoodZZ", &LikelihoodZZ);
...
vecVar[1]=LikelihoodZH135;

vecVar[2]=BDTZZ;

vecVar[3]=LikelihoodZZ;
         for (std::map<std::string,int>::iterator it = Use.begin(); it != Use.end(); it++) {
            if (it->second) {
