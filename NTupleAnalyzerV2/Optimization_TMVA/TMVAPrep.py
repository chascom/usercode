...
#os.system('rm -r /afs/cern.ch/work/'+person[0]+'/'+person+'/TMVA_temp/tmpfiles/tmva_scratch')
os.system('mkdir /tmp/'+person+'/tmva_scratch')
#os.system('mkdir /afs/cern.ch/work/'+person[0]+'/'+person+'/TMVA_temp/tmpfiles/tmva_scratch')
for stype in signaltags:
	...
	for signal in signals:
		...
		#tmpdir = '/afs/cern.ch/work/'+person[0]+'/'+person+'/TMVA_temp/tmpfiles/tmva_scratch/'+signal
		...
		for line in f:
			...
			if '"!H:!V:NTrees=400' in line:
				#line = line.replace('NTrees=400','NTrees=500') #NTrees = 2000
				#line = line.replace('MaxDepth=3','MaxDepth=3') #MaxDepth=4
				line = line.replace('NTrees=400',String_NTrees) #NTrees = 2000
				line = line.replace('MaxDepth=3',String_MaxDepth) #MaxDepth=4

			if ":NSmoothSig[0]=20:NSmoothBkg[0]" in line:
				line = line.replace(':NSmoothSig[0]=20:NSmoothBkg[0]=20',String_NSmooth)
				
			if "NAvEvtPerBin=50" in line: #likelihood
				line = line.replace('NAvEvtPerBin=50','NAvEvtPerBin=50')
				
			if "NSmoothBkg[1]=10" in line:
				line = line.replace('NSmoothBkg[1]=10','NSmoothBkg[1]=10')
