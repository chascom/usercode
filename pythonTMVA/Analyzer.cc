...
int main(int argc, char* argv[])
{
    ...
    bool isSingleMuPD(!isMC && url.Contains("SingleMu") && !url.Contains("DoubleMu"));
    ...
    fprintf(outTxtFile_final,"run lumi event passId,passIdAndIso,passZmass,passZpt,pass3dLeptonVeto,passBveto,passLMetVeto,passdphiZllmetCut,passMet,passBalanceCut,passMTcut evCat | jet1.pt jet1.eta jet1.btag pfMET | (wdecay,zdecay)\n");
    ...
    //data-driven Z+jets reweighting file
    ...
    TString fitParametersFile = "/afs/cern.ch/work/c/chasco/CMSSW_5_3_11/src/CMGTools/HtoZZ2l2nu/src/MuScleFitCorrector_v4_1/";
    if(use2011Id) {
    ...
    stmkr_sherpaup->Branch("mtzh",&mtzh,"mtzh/D");
    stmkr_sherpaup->Branch("ColinSoper",&ColinSoper,"ColinSoper/D");
    ...
    stmkr_sherpadown->Branch("mtzh",&mtzh,"mtzh/D");
    stmkr_sherpadown->Branch("ColinSoper",&ColinSoper,"ColinSoper/D");
    ...
    mon.addHistogram( new TH2F( "qdphi2lepvsqt", "; #it{q}_{T} [GeV]; #Delta#phi_{ll} [rad]; Events", 50,0,500, 50,0,TMath::Pi()) );
    ...
    // RENJIE
    //new methold of data-driven WW/ttbar/Wt/tautau background
    ...
    mon.addHistogram( new TH1F( "Ctrl_WW_zmass",";M^{ll} [GeV];Events",50,30,250) );


    //jet control
    mon.addHistogram( new TH1F("pfjetpt"       , ";p_{T} [GeV];Events",50,0,250) );
    mon.addHistogram( new TH1F("Ldjetpt"       , ";Leading Jet p_{T} [GeV];Events",50,0,250) );
    ...
    mon.addHistogram( new TH1F("pfjetstags_csv",         ";Discriminator;Events",100,0,1) );
    ...
    mon.addHistogram( new TH1F( "q_mt"  , ";M_{T}(Z,H) [GeV];Events", 85,150,1000) );
    ...
    mon.addHistogram( new TH1F( "dphi_Met_MVAMet",";#Delta#phi(MVA E_{T}^{miss},E_{T}^{miss});Events",50,0,TMath::Pi()) );
...
//Hadronic recoil correction
    ...
    if(file->IsZombie()){
    std::cout <<"IsZombie"<< std::endl;
    return -1;
    }
    std::cout <<"loop3"<< std::endl;
    if( !evSummaryHandler.attachToTree( (TTree *)file->Get(dirname) ) ) {
        file->Close();
        std::cout <<"not evSummaryHandler..."<< std::endl;
        return -1;
    }
    ...
    std::cout <<"before cnorm"<< std::endl;

    //MC normalization (to 1/pb)
    float cnorm=1.0;
    bool NewZHfile = url.Contains("MC8TeV_ZH125_");
    std::cout <<NewZHfile<<std::endl;
    TString PILEUPNAME = "evAnalyzer/h2zz/";
    if(isMC) {
        TH1F* cutflowH;
        if (NewZHfile){
            cutflowH = (TH1F *) file->Get("dataAnalyzer/llvv/cutflow");
            PILEUPNAME = "dataAnalyzer/llvv/";
        }
        else{
            cutflowH = (TH1F *) file->Get("evAnalyzer/h2zz/cutflow");
        }
    ...
    if(isMC) {
    TString puDist(PILEUPNAME+"pileuptrue");
    if(useObservedPU) puDist=PILEUPNAME+"pileup";
	...
	//	if(hasMtrigger && hasMMtrigger) continue; //m
            }
        ...
        if(isMC) {
            ...
            if(input_File.Contains("TeV_ZH")) {
                /*fprintf(outTxtFile,"ZH \n");*/ isZH=true;
            }
            if(input_File.Contains("TeV_WZ")) {
                /*fprintf(outTxtFile,"WZ \n");*/ isWZ=true;
            }
            if(input_File.Contains("TeV_ZZ")) {
                /*fprintf(outTxtFile,"ZZ \n");*/ isZZ=true;
            }
            ...
            */
            ...
            if(isZH) {
                ...
                float ewk_nloweightsZH = myZHUtils.weightNLOEWKzh(pt_zll);
            ...
            if(isWZ) {
                ...
                if(ngenleps==3 && Nneutrinos==1) {
                    ...
                    //fprintf(outTxtFile,"3Leptons:%d \t %d \t %d \t||",id_Lep0,id_Lep1,id_Lep2);
...
//            if(fabs(id1)==15 || fabs(id2)==15 || fabs(phys.leptons[ilep].id)==15) WZ3Lepsame=false; //TEST
        ...
        for(size_t ijet=0; ijet<aJets.size(); ijet++) {
            ...
            /*
                        //debuging
                        if(ev.event==49561) {
                            cout << "pt: " << aJets[ijet].pt() << "\t"
                                 << "eta: " << fabs(aJets[ijet].eta()) << "\t"
                                 << "btags: " << aJets[ijet].btag2 << endl;
                        }
            */
            ...
            mon.fillHisto("pfjetstags_csv",  tags, aJets[ijet].btag2,weight);
        }
        ...
        passRedMet=(zvvs[0].pt()>110); //ReducedMET: aRedMet.pt(); PFMET: zvvs[0].pt()
        ...
        */
        ...
        bool T_ctrl_side( passZpt && nABtags>0 && passRedMetWWCtrl);
        ...
        bool WWCtrl_RJ  ( passZpt && passRedMetWWCtrl  && passBveto /*&& passBalanceCutWWCtrl && (nAJetsGood15==0) */ && pass3dLeptonVeto);
        ...
        if(T_ctrl_side/*WWCtrl_RJ*/)   mon.fillHisto("Ctrl_WW_zmass",tags,zll.mass(),weight);
	...
	//#############################
        ...
        if(passZmass && passZpt && pass3dLeptonVeto && passBveto && passLMetVeto && passdphiZllmetCut) mon.fillHisto("zmass_WtCtrl_5",tags,zll.mass(),weight);
        if(passZmass && passZpt && pass3dLeptonVeto && passBveto && passLMetVeto && passdphiZllmetCut && passRedMet) mon.fillHisto("zmass_WtCtrl_6",tags,zll.mass(),weight);
        if(passZmass && passZpt && pass3dLeptonVeto && passBveto && passLMetVeto && passdphiZllmetCut && passRedMet && passBalanceCut && passMTcut) mon.fillHisto("zmass_WtCtrl_7",tags,zll.mass(),weight);
	...
	//############################
        ...
        if(passZmass) {
            ...
            if(passZpt) {
                ...
                //Lepton veto uncertainty
                if (passJetveto && passBveto && passDphijmet && passLMetVeto && passdphiZllmetCut &&
                        passRedMet && passBalanceCut && passMTcut) {
                    if(isWZ) {
                        mon.fillHisto("thirdleptoneta",   tags,leadingLep.eta()   ,weight);
                        if(WZ3Lepsame && extraLeptonsP4.size() == 1) {
                ...
                if(pass3dLeptonVeto) {
                    ...
                    if(passJetveto) { //set passJetveto always true
                        ...
                        if(passBveto) {
                            ...
                            if(aGoodIdJets.size()>0) mon.fillHisto("Ldjetpt",  tags, aGoodIdJets[0].pt(),weight);
                            ...
                            if(passDphijmet && passLMetVeto) {
                                ...
                                mon.fillHisto("q_mt",tags, aMT, weight);
                                    ...
                                    if(passRedMet) {
                                        ...
                                        if(passBalanceCut) {   //RJ
                                            ...
                                            if(passMTcut) { //RJ
                                                ...
                                                //RJ
                                                mon.fillHisto("dphi_Met_MVAMet",tags,dphiMet_mvaMet,weight);
                                                ...
                                                mon.fillHisto("qdphi2lepvsqt", tags, zll.pt(),dphi2l, weight);

                                                //new methold of data-driven WW/ttbar/Wt/tautau background
                                                //mon.fillHisto("zmass_WtCtrl",tags,zll.mass(),weight);
						    ...
						    std::pair<int,int> WZdecayMode = getWZdecayMode(ev);
                                                   // fprintf(outTxtFile_final,"%d | %d |  %d | %d%d%d%d%d%d%d%d%d%d%d | %s | (%d,%d)\n",ev.run,ev.lumi,ev.event,
                                                    //      passId,passIdAndIso,passZmass,passZpt,pass3dLeptonVeto,passBveto,passLMetVeto,passdphiZllmetCut,
                                                      //    passRedMet,passBalanceCut,passMTcut, evCat.Data(),WZdecayMode.first,WZdecayMode.second);
        ...
        for(size_t ivar=0; ivar<nvarsToInclude; ivar++) {
            ...
            bool passLocalBalanceCutFrozen(true);

            bool passredMetCutOpt(true); //for opt
            bool passZmetphiCutOpt(true);
            bool passLocalBalanceCutOpt(true);
            bool passmetCutOpt(true);
            ...
            double dphi12 = fabs(deltaPhi(lep2.phi(),lep1.phi()));
            double dphiZmet=fabs(deltaPhi(zll.phi(),zvv.phi()));
            bool passLocalLMetVeto(true);

            passLocalBalanceCut=(zvv.pt()/zll.pt()>0.8 && zvv.pt()/zll.pt()<1.2);
            passLocalBalanceCutFrozen=(zvv.pt()/zll.pt()>0.75 && zvv.pt()/zll.pt()<1.25);
            bool passZmetphiCutFrozen = (dphiZmet > 2.7);
            bool passmetCutFrozen = (zvv.pt() > 120);
            ...
            passredMetCutOpt = (redMet.pt() > 70); //for opt
            passZmetphiCutOpt = (dphiZmet > 0.6);
            passLocalBalanceCutOpt =(zvv.pt()/zll.pt()>0.75 && zvv.pt()/zll.pt()<1.25);
            passmetCutOpt = (zvv.pt() > 120);
            ...
            //////////// TREE VARIABLES
            ///////////////////////////
    //bool passFullSelection (isSameFlavor && passZmass && passLocalJetveto && pass3dLeptonVeto && (redMet.pt() > 65) && passLocalDphijmet && passZpt && passLocalBalanceCut && passLocalBveto);// && (zll.pt() > 50.0));// && passRedMet);//(redMet.pt() > 70));

    //bool passFullSelection (isSameFlavor && passZmass && passLocalJetveto && pass3dLeptonVeto && (redMet.pt() > 110) && passLocalBalanceCut && passLocalBveto && (dphiZmet > 2.6));
    bool passFullSelection (passIdAndIso & isSameFlavor && passZmass && passLocalJetveto && pass3dLeptonVeto && passRedMet && passLocalBalanceCut && passLocalBveto && (dphiZmet > 2.6));
    bool passFrozenSelection (passIdAndIso & isSameFlavor && passZmass && passLocalJetveto && pass3dLeptonVeto && passLocalBveto && passZmetphiCutFrozen && passmetCutFrozen && passLocalBalanceCutFrozen);
    bool passOptSelection (passIdAndIso & isSameFlavor && passZmass && passLocalJetveto && pass3dLeptonVeto && passLocalBveto && passredMetCutOpt && passZmetphiCutOpt && passLocalBalanceCutOpt);
      ...
      if(passOptSelection)
      {
        ...
        //print even info for debugging
        if (ivar == 0 && !evCat.Contains("geq2jets")) {
                fprintf(outTxtFile_final,"%d | %d |  %d | %s\n",ev.run,ev.lumi,ev.event, evCat.Data());
        }
