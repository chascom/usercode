...
def globalfiles(pathtorun):
    filestring = os.popen("cmsLs "+pathtorun+" | awk '{print $5}'").read()
    filelist = filestring.split("\n")
    filelistpure = []
    for f in filelist:
        if (".dat" in f) or (".root" in f):
            filelistpure.append(f)
    if (len(filelistpure) > 50): #truncate list, max is 255
        filelistpure = filelistpure[:50]
    filelistpureSTRING =""
    for f in filelistpure:
        if (f == filelistpure[-1]):
            filelistpureSTRING += "'" + f + "'\n"
        else:
            filelistpureSTRING += "'" + f + "',\n"
    return filelistpureSTRING
...
def run_validation(run, nevents, dset):
    ...
    if Global:
        symbol_map_cfg = { 'NEVENT':nevents, 'GLOBALTAG':GlobalTag, "OUTFILE":outFileName, "INPUTFILES":globalfiles(dset) }
    ...
    CMSRUN = os.popen("cmsRun "+cfgFileName).readlines()
    for cr in CMSRUN:
        if "Events in" in cr:
            nevents = cr
    print "after cmsRun"
    nevents = nevents.replace("Events in ","")
    print nevents, "$"*100
    symbol_map_html = { 'RUNNUMBER':run, 'NEVENT':nevents, "DATASET":dset, "CMSSWVERSION":Release, "GLOBALTAG":GlobalTag, "DATE":Time }
    htmlFile = open("Summary.html", 'a')
    replace(symbol_map_html,templateHTMLFile, htmlFile)
...
print "not done?", theFile not in oldrunlist

if theFile not in oldrunlist:
    print "before runval"
    isOK = run_validation(theFile.split("/run")[-1].replace("/",""),"N/A",theFile)
    # print "isOK", isOK
    isOK = 1
    if isOK:
        print 'Writing to ',runFile
        oldruns=open(runFile,'a')
        print 'Adding line',theFile
        #os.system('echo "'+theFile+'">>'+runFile)
        # with open(runFile,"a+") as fW:
        #     fW.write(theFile+"\n")
        oldruns.write(theFile+"\n")
        oldruns.close()
        print 'Line added?'
else:
