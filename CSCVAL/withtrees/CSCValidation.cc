...
void CSCValidation::doStandalone(Handle<reco::TrackCollection> saMuons){
 ...
 //if (writeTreeToFile && mTreeCount < 1500000){
      histos->fillmTree(nSAMuons);
      //mTreeCount++;//chasco
    //} 
  ...
  for(reco::TrackCollection::const_iterator muon = saMuons->begin(); muon != saMuons->end(); ++ muon ) {
    ...
    float innerGlobalPolarAngle = muon->theta(); //chasco
    float phiAngle = muon->phi(); //chasco
    float etaAngle = muon->eta(); //chasco
    ...
    //Fill the branch //chasco
    if (nSAMuons == 2){
    if (writeTreeToFile && staxTreeCount < 1500000){
      histos->fillStaxTree(preco, ptreco, innerGlobalPolarAngle, phiAngle, etaAngle, nCSCHits, chi2);
      //histos->fill1DHist(preco,"trP","STA Muon Momentum",100,0,300,"stax");
      //histos->fill1DHist(ptreco,"trPT","STA Muon pT",100,0,40,"stax");
      staxTreeCount++;
    }  
    }
