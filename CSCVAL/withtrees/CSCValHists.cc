...
using namespace std;
  ...
  void CSCValHists::setupTrees(){
    ...
    staxTree = new TTree("staxMomentum","sta muon momentum"); //chasco
    mTree = new TTree("m","M");

    // Create a branch on the tree
    rHTree->Branch("rHpos",&rHpos.endcap,"endcap/I:station/I:ring/I:chamber/I:layer/I:localx/F:localy/F:globalx/F:globaly/F");
    segTree->Branch("segpos",&segpos.endcap,"endcap/I:station/I:ring/I:chamber/I:layer/I:localx/F:localy/F:globalx/F:globaly/F");
    staxTree->Branch("staxpos",&staxpos.p,"p/F:pt/F:theta/F:ota/F:eta/F:nCSCHits/I:chi2/F"); //chasco
    mTree->Branch("mpos",&mpos.NSAM,"NSAM/I");
  }
  ...
  void CSCValHists::fillStaxTree(float p, float pt, float theta, float ota, float eta, int nCSCHits, float chi2){

    //Fill the stax branch //chasco
    staxpos.p = p;
    staxpos.pt = pt;
    staxpos.theta = theta;
    staxpos.ota = ota;
    staxpos.eta = eta;
    staxpos.nCSCHits = nCSCHits;
    staxpos.chi2 = chi2;
    staxTree->Fill();

  }

  void CSCValHists::fillmTree(int NSAM){
   
   //Fill the m branch //chasco
   mpos.NSAM = NSAM;
   mTree->Fill();
