...
class CSCValHists{
  ...
  // fill the stax tree //chasco
  void fillStaxTree(float p, float pt, float theta, float ota, float eta, int nCSCHits, float chi2);

  // fill the m tree //chasco
  void fillmTree(int NSAM);
  ...
  //A struct for creating a Tree/Branch of stax info //chasco
  struct staxRecord {
    float p;
    float pt;
    float theta;
  ...
  //A struct for creating a Tree/Branch of NSAM into //chasco
  struct mRecord {
   int NSAM;
  } mpos;
