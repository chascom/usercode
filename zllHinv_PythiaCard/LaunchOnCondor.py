...
CopyRights += '#             Feb 2014             #\n'
CopyRights += '#   matthew.ervin.chasco@cern.ch   #\n'
CopyRights += '#             Aug 2014             #\n'
CopyRights += '####################################\n'
...
def CreateTheShellFile(argv,OutputDir=''):
	global Path_Shell
	global Path_Cfg
	global Path_Ntuple
	global CopyRights	
...
#	        shell_file.write('eval `scramv1 runtime -sh`\n')
		...
		shell_file.write('cmsRun ' + os.getcwd() + '/'+Path_Ntuple +' file:Events_%04i.root'%(Jobs_Count)+'\n')
		if(len(OutputDir)>0):
			#shell_file.write('cmsStage -f /tmp/Events_%04i.root %s/ \n'%(Jobs_Count,OutputDir))
			#shell_file.write('rm /tmp/Events_%04i.root \n'%Jobs_Count)
			# shell_file.write('cmsRun ' + os.getcwd() + '/'+Path_Ntuple +" file:Events_%04i.root"+'\n'%(Jobs_Count,OutputDir))
			shell_file.write('cp ntuple_Events_%04i.root %s/ \n'%(Jobs_Count,OutputDir))
	else:
...
def AddJobToCmdFile():
	...
	if useLSF:
		...
		##RJ
		cmd_file.write("bsub -R \"pool>10000\" -sp 99 -q " + Jobs_Queue + " -o " + Path_Log + ".log "+ " -J " + Jobs_Name+Jobs_Index + " '" + os.getcwd() + "/"+Path_Shell + " 0 ele'\n")
	else:
        	cmd_file.write('\n')
...
def SendCluster_Push(Argv,OutputDir=''):
        global Farm_Directories
        global Jobs_Count
        global Jobs_Index
	global Path_Shell
	...
	Jobs_Index = "%04i_" % Jobs_Count
        if Jobs_Count==0 and (Argv[0]=="ROOT" or Argv[0]=="FWLITE"):                
                ...
                CreateTheShellFile([Argv[0],Argv[1]],OutputDir)
                os.system('sh '+Path_Shell)
                os.system('rm '+Path_Shell)
		print "Getting the jobs..."
	print Argv
        CreateTheShellFile(Argv,OutputDir)
        AddJobToCmdFile()
