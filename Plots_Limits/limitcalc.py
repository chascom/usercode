...
os.chdir("/afs/cern.ch/work/c/chasco/CMSSW_5_3_3_patch2/src/HiggsAnalysis/")
...
if (runlimits):
	...
	os.chdir(CL)
	prefit = "--expectSignal=1 -t -1"
	def savetables(filelist):
		savelimits = []
		if "count" in str(filelist):
			b= "count"
		else:
			b = "shape"
		RMF = open("limitList_"+b+".txt",'w')
		RMF2 = open("limitTable"+b+".txt",'w')
		EE= ""
		MM= ""
		HIST = ""
		CARD = ""
			...
			for l in k:
				...
				if "Expected 50.0%: r < " in l:
					if j > float(l.split(" ")[-1]):
						j = float(l.split(" ")[-1])
						minname = c
					if ("EE" in c) and ("MM" not in c):
						EE = EE + " " + str(round(float(l.split(" ")[-1])*100.0))
						EEc = EEc + " " + c
					if ("MM" in c) and ("EE" not in c):
						MM = MM + " " + str(round(float(l.split(" ")[-1])*100.0))
						MMc = MMc + " " + c
					if ("LL" in c):
						HIST = HIST + " " + str(round(float(l.split(" ")[-1])*100.0))
						HISTc = HISTc + " " + c
					if ("MM" in c) and ("EE" in c):
						CARD = CARD + " " + str(round(float(l.split(" ")[-1])*100.0))
						CARDc = CARDc + " " + c
			RMF.write("\n")
			RMF.write("-"*40+"\n")
		RMF2.write(EE + "\n")
	...
	savetables(countfiles)
	savetables(shapefiles)
	# RMF = open("limitList.txt",'w')
	# for x in range(len(savelimits)):
	# 	RMF.write(cardfiles(x)+"\n")
	# 	RMF.write("\n")
