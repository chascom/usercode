...
def CumulativeBin(inputhisto,cutoff,name):
	...
	print newconts
	print "-"*20
	hout= TH1F(name,name,len(newbins)-1,array.array('d',newbins)) # Declar empty TH1F
...
def MakePlot(names_nonorder,variableOrig,bin,_BoundariesOfBins,logy,Application,InjectSignal,FitScale,STAGE,BkgdZH,curvefit,imageout,AdditionalCut,ACS):#plot making function
	BoundariesOfBins = [x for x in _BoundariesOfBins]
	variable = variableOrig.replace("_REG","")
	variable_string = variable.replace("*","").replace("(","").replace(")","").replace(" ","").replace(".","p").replace("/","div").replace("-","minus").replace("math.","").replace(",","")
	if variable_string == "met":
		variable_string = "PF" + variable_string
	variable_string = variable_string + '_' + ACS
	print variable, bin, BoundariesOfBins, AdditionalCut, ">"*40
	...
	_min = BoundariesOfBins[0]
	print _min, "MIN"
	_max = BoundariesOfBins[-1]
	print _max, "MAX"
	#TeV8 = False
	if TeV8:
		Luminosity = 19600
	...
	bin = int(bin)
	for n in names:
		#print('oh_'+n+'=TH1F("oh_'+n+'","",bin,_min,_max)')
		#print bin, _min, _max
		exec('oh_'+n+'=TH1F("oh_'+n+'","",bin,_min,_max)')#make histograms for each file
		exec('oh_'+n+'.Sumw2()')
	...
	#(abs(l1phi-l2phi) > pi)*(2*pi - abs(l1pt - l2pt)) + (abs(l1phi-l2phi) <= pi)*abs(l1pt-l2pt)
	# if "tranversemass" in variable:
	# 	variable = "sqrt(2*l1pt*l2pt*(1-cos((abs(l1phi-l2phi) > pi)*(2*pi - abs(l1pt - l2pt)) + (abs(l1phi-l2phi) <= pi)*abs(l1pt-l2pt))))"
	# if "llphi" in variable:
	# 	variable = "(abs(l1phi-l2phi) > pi)*(2*pi - abs(l1pt - l2pt)) + (abs(l1phi-l2phi) <= pi)*abs(l1pt-l2pt)"
		#variable = "sqrt(2*l1pt*l2pt*(1-cos(TMath.DeltaPhi(l1phi,l2phi))))"
	...
	#OH_Data=TH1F("OH_Data",variable+" "+inj+" "+str(Application)+STAGE,bin,_min,_max)
	OH_Data=TH1F("OH_Data",variable+" "+ACS+STAGE,bin,_min,_max)
	OH_Data.Sumw2()
	#H_Data2=TH1F("H_Data2",variable+" "+inj+" "+str(Application),bin,min,max)
	#H_Data2.Sumw2()
	H_Bkgd = TH1F("H_Bkgd","",bin,_min,_max)
	H_Bkgd.Sumw2()
	H_Sign = TH1F("H_Sign","",bin,_min,_max)
	H_Sign.Sumw2()
	...
	H_Sign_Inj = TH1F("H_Sign_Inj","",bin,_min,_max)
	if (InjectSignal):
	...
	##### make S/sqrt(B) plot
	pad3.cd()
	h_SB = TH1F("h_SB","",bin,_min,_max) #make plot to look at ratios of MC and DATA bins
	H_MCB = TH1F("H_MCB","",bin,_min,_max)
	H_MCB.Sumw2()
	H_MCS = TH1F("H_MCS","",bin,_min,_max)
	H_MCS.Sumw2()
	...
	# from RMF import *
	RearrangeMatrixFile = "RMF_"+variable_string+".py"
	bin_array = []
	...
	RH_Sign = TH1F("RH_Sign","",bin,_min,_max)
	RH_Sign.Sumw2()
	#print "BIN ARRAY", bin_array
	...
	if AutoAdj:
		BinBoundsFile = "BinBounds_"+variable_string+".py"
		if (STAGE == ""):
			Cbin = CumulativeBin(RH_Sign,2.0,"H_Sign_new")
			H_Sign_new = Cbin[0]
	...
	h_SingleT=TH1F("h_SingleT","",bin,_min,_max) #group SingleT samples together
	h_SingleT.Sumw2()

	for n in names:
		## print('h_'+n+'=TH1F("h_'+n+'","",bin,min,max)')
		exec('h_'+n+'=TH1F("h_'+n+'","",bin,_min,_max)')#make histograms for each file
		## print "DONE" + "-"*20
		exec('h_'+n+'.Sumw2()')
	...
	H_Data=TH1F("H_Data",""+rearrbool*"Rearranged "+variable+" "+str(Application)+STAGE+inj,bin,_min,_max)
	H_Data.Sumw2()
	...
	FileFF=TFile.Open("ShapeFiles/Shape_Card_input_"+variable_string+STAGE+".root","RECREATE")
	...
	if makestackplots:
		...
		if ("BDT" in variable) or ("Likelihood" in variable):
			H_Data.GetXaxis().SetTitle(variable_string)
			H_Data.GetYaxis().SetTitle("Number of Events/" + str((_max-_min)/bin))
		else:
			H_Data.GetXaxis().SetTitle(variable_string + ' (GeV)')
			H_Data.GetYaxis().SetTitle("Number of Events/" + str((_max-_min)/bin) + " GeV")
		...
		h_comp = TH1F("h_comp","",bin,_min,_max) #make plot to look at ratios of MC and DATA bins
		H_MC = TH1F("H_MC","",bin,_min,_max)
		H_MC.Sumw2()
		H_X = TH1F("H_X","",bin,_min,_max)
		H_X.Sumw2()
		...
		# else:
		#c2.Print(imageout+'/'+variable_string+str(Application)+'_'+inj+STAGE+'_lin'*(1-logy)+'_'+str(curvefit)+'.png')
		c2.Print(imageout+'/'+variable_string+'_'+TeV8*'8TeV'+(1-TeV8)*'7TeV'+STAGE+'_lin'*(1-logy)+'.png')
		#First = False
...
sdir = predir+'Apr23_8TeV/'
TeV8 = True
#sdir = predir+'mar25_8TeV/'
...
#variables = ["met_REG"]
#tranversemass = "math.sqrt(2*l1pt_REG*l2pt*(1-math.cos(abs(DeltaPhi(l1phi,l2phi)))))"
#variables = ["llphi_REG","met_REG","REDmet_REG","Zmetphi_REG","met_REG/zpt_REG","nj15_REG"]

default = ""
additionalCutA = "_COMBO"
additionalCut_str = additionalCutA.replace(">","_gt_").replace("<","_lt_").replace("*","_by_").replace("(","").replace(")","").replace(" ","").replace(".","p").replace("/","_div_").replace("+","_plus_").replace("-","_minus_")

# regionA = "*((met/zpt)>0.8)*((met/zpt)<1.2)*(Zmetphi > 2.9)*(nj15 < 2)*(met > 60)"
# regionB = "*((met/zpt)>0.8)*((met/zpt)<1.2)*(Zmetphi <= 2.9)*(nj15 < 1)*(met > 60)"
# regionC = "*(((met/zpt)<=0.8)+((met/zpt)>=1.2))*(Zmetphi > 2.9)*(nj15 < 1)*(met > 60)"
# regionD = "*(((met/zpt)<=0.8)+((met/zpt)>=1.2))*(Zmetphi <= 2.9)*(nj15 < 1)*(met > 60)"
EE = "*(finstate > 1.5)"
MM = "*(finstate < 1.5)"

regionAo = "*((met/zpt)>0.8)*((met/zpt)<1.2)*(Zmetphi > 2.8)*(Zmetphi - 0.75*(met/zpt) > 2.3)" #*(Zmetphi + 0.75*(met/zpt) <= 4.4)"
regionAp = "*((met/zpt)>0.8)*((met/zpt)<1.2)*(Zmetphi > 2.8)*(Zmetphi - 0.75*(met/zpt) <= 2.3)" #*(Zmetphi + 0.75*(met/zpt) > 4.4)"
regionA = "*((met/zpt)>0.8)*((met/zpt)<1.2)*(Zmetphi > 2.8)"
regionB = "*((met/zpt)>0.8)*((met/zpt)<1.2)*(Zmetphi <= 2.8)"
regionC = "*(((met/zpt)<=0.8)+((met/zpt)>=1.2))*(Zmetphi > 2.8)"
regionD = "*(((met/zpt)<=0.8)+((met/zpt)>=1.2))*(Zmetphi <= 2.8)"
regionE = regionA
regionF = regionA + "*(met > 70)"
regionG = regionA + "*(met > 70)*pLepVeto"
regionH = regionA + "*(met > 70)*pLepVeto*(nj20 < 1.0)"
regionI = regionA + "*(met > 90)*pLepVeto*(nj20 < 1.0)"
regionJ = regionA + "*(met > 105)*pLepVeto*(nj30 < 1.0)"
regionK = regionA + "*(met > 110)*pLepVeto*(nj30 < 1.0)"
regionL = regionA + "*(met > 115)*pLepVeto*(nj30 < 1.0)"
regionM = regionA + "*(met > 120)*pLepVeto*(nj30 < 1.0)"
regionR = "*(zpt > 80.0)*(Zmetphi > 2.6)*(REDmet > 110)*((met/zpt)>0.4)*((met/zpt)<1.8)"
#regionS = regionR + "*(sqrt(2*met*zpt*(1-cos(Zmetphi))) > 220.0)"
regionS = regionR + "*(TransMass3 > 220.0)"
regionT = regionS + "*((met/zpt)>0.8)*((met/zpt)<1.2)"
regionT10 = regionS + "*(abs(mass-91)<10)"
regionT11 = regionS + "*(abs(mass-91)<11)"
regionT12 = regionS + "*(abs(mass-91)<12)"
regionT13 = regionS + "*(abs(mass-91)<13)"
regionT14 = regionS + "*(abs(mass-91)<14)"
regionO = "*(abs(mass-91)<7.5)*(zpt>45.0)*(REDmet > 65.0)*((met/zpt)>0.4)*((met/zpt)<1.8)*pDphijmet"

regionX = "*(abs(mass-91)<17.5)*(zpt > 80.0)*(Zmetphi > 2.6)*(REDmet > 110.0)*((met/zpt)>0.8)*((met/zpt)<1.2)"
regionMT = regionX + "*(TransMass3 > 220.0)"
variable_cuts_bins = []
# variable_cuts_bins.append(["llphi_REG",default,[0.0,3.2],16])
#variable_cuts_bins.append(["met_REG",default,[0.0,700.0],35])
# variable_cuts_bins.append(["REDmet_REG",default,[0.0,700.0],35])
# variable_cuts_bins.append(["met_REG/zpt_REG",default,"_no_cuts",[0.0,2.0],40])
# variable_cuts_bins.append(["Zmetphi_REG",default,"_no_cuts",[0.0,3.2],16])
# variable_cuts_bins.append(["nj15_REG",default,[0.0,10.0],10])

# variable_cuts_bins.append(["met_REG",regionA,"regionA",[0.0,340.0],17])
# variable_cuts_bins.append(["met_REG",regionB,"regionB",[0.0,340.0],17])
# variable_cuts_bins.append(["met_REG",regionC,"regionC",[0.0,340.0],17])
# variable_cuts_bins.append(["met_REG",regionD,"regionD",[0.0,340.0],17])



#variable_cuts_bins.append(["met/zpt_REG","","all",[0.0,2.0],40])
#variable_cuts_bins.append(["Zmetphi_REG","","all",[0.0,3.2],32])regionI = regionA + "*(met > 90)*pLepVeto*(nj20 < 1.0)"
vari = "met_REG"
#vari2 = "sqrt(2*met_REG*zpt_REG*(1-cos(Zmetphi_REG)))"
vari2 = "TransMass3_REG"
# variable_cuts_bins.append([vari,regionAo,"regionAo",[0.0,10],10])
# variable_cuts_bins.append([vari,regionAp,"regionAp",[0.0,10],10])
# variable_cuts_bins.append([vari,regionA,"regionA",[0.0,10],10])
# variable_cuts_bins.append([vari,regionB,"regionB",[0.0,10],10])
# variable_cuts_bins.append([vari,regionC,"regionC",[0.0,10],10])
# variable_cuts_bins.append([vari,regionD,"regionD",[0.0,10],10])
# variable_cuts_bins.append([vari,regionE,"regionE",[0.0,10],10])
# variable_cuts_bins.append([vari,regionK,"regionJ",[0.0,10],10])
#variable_cuts_bins.append([vari2,regionK,"regionK",[0.0,220.0,550.0,720,1000.0,1220.0,1500.0],300])
#variable_cuts_bins.append([vari2,regionM,"regionM",[0.0,220.0,550.0,720,1000.0,1220.0,1500.0],300])
# variable_cuts_bins.append(["zpt_REG",regionK,"regionK",[0.0,22.0,55.0,72.0,110.0,142.0,165.0,182.0,220.0,252.0,275.0,1000.0],17])
# variable_cuts_bins.append(["zpt_REG",regionM,"regionM",[0.0,22.0,55.0,72.0,110.0,142.0,165.0,182.0,220.0,252.0,275.0,1000.0],17])
# variable_cuts_bins.append([vari,regionM,"regionM",[0.0,22.0,55.0,72.0,110.0,142.0,165.0,182.0,220.0,252.0,275.0,1000.0],17])
# variable_cuts_bins.append([vari,regionL,"regionL",[0.0,55.0,110.0,165.0,220.0,275.0,1000.0],17])
# variable_cuts_bins.append([vari,regionL,"regionJ",[0.0,55.0,110.0,165.0,220.0,275.0,1000.0],17])
regionR1 = regionR
regionR2 = regionR
regionR3 = regionR
qqq = [100.0,200.0,300.0,400.0,500.0,600.0,700.0,800.0,900.0,1000.0,1100.0,1200.0,1300.0,1400.0,1500.0]
# variable_cuts_bins.append([vari2,regionK,"regionK",qqq,40])
# variable_cuts_bins.append([vari2,regionM,"regionM",qqq,40])
#variable_cuts_bins.append([vari2,regionX + EE,"regionX_EE",qqq,40])
variable_cuts_bins.append([vari2,regionMT + EE,"regionMT_EE",qqq,70])
#variable_cuts_bins.append([vari2,regionX + MM,"regionX_MM",qqq,40])
variable_cuts_bins.append([vari2,regionMT + MM,"regionMT_MM",qqq,70])
variable_cuts_bins.append([vari2,regionMT,"regionMT_LL",qqq,70])
# variable_cuts_bins.append(["TransMass_REG",regionR1,"regionR",qqq,40])
# variable_cuts_bins.append(["TransMass2_REG",regionR2,"regionR",qqq,40])
# variable_cuts_bins.append(["TransMass3_REG",regionR3,"regionR",qqq,40])
# variable_cuts_bins.append([vari2,regionS,"regionS",qqq,40])
# variable_cuts_bins.append([vari2,regionT,"regionT",qqq,40])
# variable_cuts_bins.append([vari2,regionT14,"regionT14",qqq,40])
# variable_cuts_bins.append([vari2,regionT13,"regionT13",qqq,40])
# variable_cuts_bins.append([vari2,regionT12,"regionT12",qqq,40])
# variable_cuts_bins.append([vari2,regionT11,"regionT11",qqq,40])

# variable_cuts_bins.append(["llphi_REG",regionA,"regionA",[0.0,3.2],32])
# variable_cuts_bins.append(["llphi_REG",regionB,"regionB",[0.0,3.2],32])
# variable_cuts_bins.append(["llphi_REG",regionC,"regionC",[0.0,3.2],32])
# variable_cuts_bins.append(["llphi_REG",regionD,"regionD",[0.0,3.2],32])

# variable_cuts_bins.append(["zpt_REG",regionA,"regionA",[0.0,340.0],17])
# variable_cuts_bins.append(["zpt_REG",regionB,"regionB",[0.0,340.0],17])
# variable_cuts_bins.append(["zpt_REG",regionC,"regionC",[0.0,340.0],17])
# variable_cuts_bins.append(["zpt_REG",regionD,"regionD",[0.0,340.0],17])

# variable_cuts_bins.append(["l1pt_REG",regionA,"regionA",[0.0,340.0],17])
# variable_cuts_bins.append(["l1pt_REG",regionB,"regionB",[0.0,340.0],17])
# variable_cuts_bins.append(["l1pt_REG",regionC,"regionC",[0.0,340.0],17])
# variable_cuts_bins.append(["l1pt_REG",regionD,"regionD",[0.0,340.0],17])

# variable_cuts_bins.append(["l2pt_REG",regionA,"regionA",[0.0,340.0],17])
# variable_cuts_bins.append(["l2pt_REG",regionB,"regionB",[0.0,340.0],17])
# variable_cuts_bins.append(["l2pt_REG",regionC,"regionC",[0.0,340.0],17])
# variable_cuts_bins.append(["l2pt_REG",regionD,"regionD",[0.0,340.0],17])

# variable_cuts_bins.append(["nj15_REG",regionA,"regionA",[0.0,10.0],10])
# variable_cuts_bins.append(["nj15_REG",regionB,"regionB",[0.0,10.0],10])
# variable_cuts_bins.append(["nj15_REG",regionC,"regionC",[0.0,10.0],10])
# variable_cuts_bins.append(["nj15_REG",regionD,"regionD",[0.0,10.0],10])


#variables = ["met_REG/zpt_REG"]
#variables = ["zpt"]
# #additionalCut = '*(met > 80)*1.0'
# #additionalCut = "*((met/zpt)>0.8)*((met/zpt)<1.2)*(Zmetphi > 2.6)"#*(REDmet > 75)"
# additionalCut = "*(Zmetphi > 2.9)*((met/zpt)>0.8)*((met/zpt)<1.25)*(REDmet > 90)"
# additionalCut = "*(Zmetphi > 2.8)*((met/zpt)>0.8)*((met/zpt)<1.1)*(met > 90)"
# additionalCut = "*(met > 65)"#*((met/zpt)>0.9)*((met/zpt)<1.2)"#*(Zmetphi > 2.7)"
# additionalCut = "*((met/zpt)>0.85)*((met/zpt)<1.2)*(Zmetphi > 2.9)*(met > 80)*(abs(mass-91)<10)"
# additionalCut = "*((met/zpt)>0.8)*((met/zpt)<1.2)*(abs(mass-91)<10)*(Zmetphi > 2.8)"
# additionalCut = "*(Zmetphi > 2.9)*((met/zpt)>0.8)*((met/zpt)<1.2)"
# additionalCut_str = additionalCut.replace(">","_gt_").replace("<","_lt_").replace("*","_and_").replace("(","").replace(")","").replace(" ","").replace(".","p").replace("/","_div_").replace("+","_plus_").replace("-","_minus_")
# imgout = 'TMVA_test/WIDE8_'+additionalCut_str
# #additionalCut = ''
...
os.system("rm ShapeFiles/Shape_Card_input*.root")

for x in Suffix:
	for v in variable_cuts_bins:
		if "_REG" in v[0]:
			#additionalCut_str = v[1].replace(">","_gt_").replace("<","_lt_").replace("*","_and_").replace("(","").replace(")","").replace(" ","").replace(".","p").replace("/","_div_").replace("+","_plus_").replace("-","_minus_")
			#imgout = 'TMVA_test/H8_'+additionalCut_str
			if TeV8:
				imgout = 'TMVA_test/H8_'+additionalCutA+"_"+v[2]
			else:
				imgout = 'TMVA_test/H7_'+additionalCutA+"_"+v[2]
			Lphy = MakePlot(names,v[0],v[-1],v[-2],True,True,Inject,True,x,'nope',CurveFit,imgout,v[1],v[2])
		else:
			print "TMVA"
...
# 	for v in variables:
# 		if "_REG" in v:
# 			if ("Zmetphi" in v) or ("llphi" in v):
# 				zptBins = [0.0,3.2]
# 				print zptBins
# 				Lphy = MakePlot(names,v,16,zptBins,True,True,Inject,True,x,'nope',CurveFit,imgout,additionalCut)
# 			#Lphy = MakePlot(names,v,30,30.0,700.0,zptBins,False,True,5035,Inject,True,x,'nope',CurveFit,imgout,additionalCut)
# 			else:
# 				if "met_REG/zpt_REG" in v:
# 					zptBins = [0.0,2.0]
# 					Lphy = MakePlot(names,v,40,zptBins,True,True,Inject,True,x,'nope',CurveFit,imgout,additionalCut)
# 				else:
# 					zptBins = [0.0,700.0]
# 					Lphy = MakePlot(names,v,35,zptBins,True,True,Inject,True,x,'nope',CurveFit,imgout,additionalCut)
# 		else:
# 			if "ZH" in v:
# 				# print "BEFORE FUNCTION <><><><><><><<><><><><><><><><><><><><><><><><><><><><><><><><><", LikelihoodBins
# 				exec("Lf"+v+" = MakePlot(names,'LikelihoodF_"+v+"',20,0.0,1.0,LikelihoodBins,False,True,Inject,True,x,'nope',CurveFit,imgout,additionalCut)")
# 			else:
# 				# print "BEFORE FUNCTION <><><><><><><<><><><><><><><><><><><><><><><><><><><><><><><><><", LikelihoodBins
# 				exec("L"+v+" = MakePlot(names,'Likelihood"+v+"',20,0.0,1.0,LikelihoodBins,False,True,Inject,True,x,'nope',CurveFit,imgout,additionalCut)")
# 	#Lzz= MakePlot(names,'LikelihoodZZ',20,0.0,1.0,False,True,5035,Inject,True,x,'nope',CurveFit,imgout)

if TeV8:
	os.system("rm -r ShapeFiles/H8_"+additionalCut_str)
	os.system("mkdir ShapeFiles/H8_"+additionalCut_str)
else:
	os.system("rm -r ShapeFiles/H7_"+additionalCut_str)
	os.system("mkdir ShapeFiles/H7_"+additionalCut_str)



for w in variable_cuts_bins:
	v = w[0].replace("_REG","").replace("*","").replace("(","").replace(")","").replace(" ","").replace(".","p").replace("/","div").replace("-","minus").replace("math.","").replace(",","")
	
	v2 = w[2].replace("*","").replace("(","").replace(")","").replace(" ","").replace(".","p").replace("/","div").replace("-","minus").replace("math.","").replace(",","")
	
	
	if v == "met":
		v = "PF"+v
	if TeV8:
		os.system("hadd ShapeFiles/H8_"+additionalCut_str+"/"+v+v2+".root ShapeFiles/*Shape_Card_input*"+v+"*"+v2+"*.root")
	else:
		os.system("hadd ShapeFiles/H7_"+additionalCut_str+"/"+v+v2+".root ShapeFiles/*Shape_Card_input*"+v+"*"+v2+"*.root")
